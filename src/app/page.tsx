...
export default function HomePage() {
  ...
  // FIX: Mengembalikan fungsi fetch ke API asli Anda yang stabil
  const fetchSpecificVerse = useCallback(async (verseNumber: number) => {
    ...
    setTimeout(async () => {
      try {
        const response = await fetch(`https://api.alquran.cloud/v1/ayah/${verseNumber}/id.indonesian`);
        ...
        if (data.code === 200) {
          ...
          setVerse({
            ...
            translation: verseData.text.transliteration.en, // Menggunakan transliterasi dari API ini
            chapterName: getIndonesianSurahName(verseData.surah.number),
          });
  ...
  return (
    <>
      ...
      <footer className="w-full text-center text-gray-500 text-sm p-4 border-t border-gray-200 fixed bottom-0 left-0 bg-white">
